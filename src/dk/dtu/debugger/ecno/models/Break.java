	...
	public boolean isBreakpoint(Interaction interaction) {
		...
		if(elementType != null){
			...
			for(Object o : interaction.getElements()){
				if(InspectionToolEngineController.getInstance().isElementType(o, elementType)){
					hasElementType = true;
					break;
				}
			}
			if(InspectionToolEngineController.getInstance().isElementType(interaction.getTriggerElement(), elementType)){
				hasElementType = true;
			}
		}
