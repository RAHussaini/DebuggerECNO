...
import static dk.dtu.debugger.ecno.figures.ViewsUtil.*;
...
public class BreakpointView  extends ViewPart 
	...
	IBreakpointListener,
	GraphItemListener{
	public static final String ID = "dk.dtu.debugger.ecno.figures.DebuggerView";
	...
	public void createPartControl(Composite parent) {
		...
		continueBtn = createButton(addBreakpointContainer, SWT.PUSH, "Continue", 2, new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				new Thread(new Runnable() {
					@Override
					public void run() {
						InspectionToolEngineController.getInstance().continueFromBreakpoint();
					}
				}).start();
			}
		...
		interactionElements.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				IEventType[] eventTypes = InspectionToolEngineController.getInstance().getEventTypes(element.getNode());
...
//				((EventTypeImpl)interactions.get(0).getTriggerEvent()).getParameters()
		...
		InspectionToolEngineController.getInstance().addBreakpointListener(this);
		InspectionToolEngineController.getInstance().addElementListener(this);
	...
	public void addBreakpoint(){
		...
		Display.getDefault().asyncExec(new Runnable() {
			...
			public void run() {
				...
				InspectionToolEngineController.getInstance().addBreakpoint(breakpoint);
				updateScrollSize();
			}
		});
...
//		System.out.println("widget default selected: " + e.getSource());
	...
	@Override
	public void dispose() {
		InspectionToolEngineController.getInstance().removeBreakpointListener(this);
	...
	public void removeControl(Object breakpoint) {
		InspectionToolEngineController.getInstance().removeBreakpoint((IBreakpoint)breakpoint);
