...
public class OverviewView extends ViewPart implements SelectionListener, InteractionListener {
	...
	public static final String ID = "dk.dtu.debugger.ecno.figures.InteractionView";
...
//				System.out.println("default refresh graphitems selected...");
		...
		calculateInteractionsBtn.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				new Thread(new Runnable() {
					...
					@Override
					public void run() {
						InspectionToolEngineController.getInstance().calculateInteractions();	
					}
				}).start();
		...
		InspectionToolEngineController.getInstance().addInteractionListener(this);
	}
	
	private synchronized void updateGraphItemValues(){
		List<ObjectViewModel> elements = InspectionToolEngineController.getInstance().getElements();
